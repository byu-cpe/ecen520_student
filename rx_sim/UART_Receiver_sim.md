...
Create a make rule `synth_rx_gray` that will synthesize your receiver using a gray code encoding and generate a log file named `synth_rx_gray.log` for this step.
...
Create a makefile rule `sim_ssd` for this simulation that saves the simulation output to a log file named `sim_ssd.log`.
...
2. Assignment specific Questions:
    1. Fill in the state machine encoding table in your report for both the default synthesis and the gray code synthesis. To find the encoding style the synthesis tool used, search the log file for the phrase "using encoding '<style>' in module '<module>'".
    1. Fill in the resource utilization table in your report (CARRY4, LUTx, FDxx) for both the default and gray code synthesis.
    1. Provide a detailed list of the steps that the testbench follows to test your receiver. Consult the [testbench](tb_rx.sv) for details.
    1. Summarize the purpose of each 'task' in the testbench.
